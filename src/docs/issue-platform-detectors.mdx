...
Also, add the corresponding issue to the admin settings ui. It allows support to enable/disable detection of the issue for a project:
...
- The **minimum threshold value** of the range, should hold some significance when exceeded. We want our users to find a issue that we detect, significant enough to look further into. 
- The **impact** of changing the thresholds chosen to be made configurable, **should be easy for the user to interpret**. For example: Instead of displaying the raw duration of db base spans on the main thread
 as a configurable threshold, we display the frame rate drop that may result from having a db span of that duration on the main thread:
