...
_Note:_ Some customers may have both a regular plan _and_ a plan trial. If you
need to target a plan tier, (e.g., `"am2"` you should create Flagr constraints
for both the `subscription_plan-tier` property _and_ the
`subscription_plan-trial-plan-tier` property.)
