# Flagpole
Flagpole is our internal, options-backed feature flagging library. 
...
: The ISO datetime of when the feature was added to the config yaml.
...
: The global toggle for the feature flag. Defaults to `True`.

`segments`

: A wrapper around a list of conditions, acting as a logical grouping of customers/entities to enable the feature flag for. If no conditions are defined, the feature will evaluate to `True`.

### Segments
`conditions`

: A list of predicates to evaluate for the feature flag to be enabled for this segment. All conditions must be evaluate to `True` in order for the segment to be enabled. If no conditions are defined, the segment will evaluate to `False`.
