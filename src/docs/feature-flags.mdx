...
Feature flags are instantiated in Sentry, then configured in production via Flagr
...
If you're checking for more than one feature flag you should use the
FeatureManager's `batch_has` method instead.

```python
batch_features = features.batch_has(
    ['projects:test-feature3', 'projects:test-feature4'],
    actor=user,
...
Which results in a dict like this with the feature type followed by the object
id

```python
{
    'project:1': {
        'projects:test-feature3': True,
        'projects:test-feature4': True,
    },
    'project:2': {
        'projects:test-feature3': False,
        'projects:test-feature4': False,
    },
}
```
...
need to setup your feature in Flagr.

First add your feature to the flagr allowlist: in `getsentry/features.py` you
will find the Flagr feature handler (`FlagrFeatureHandler`) then add your
feature to the `allowed_features` property.

```python
allowed_features = set(
    [
        ...
        "organizations:test-feature",
    ]
)
```
...
Each flag is defaulted to disabled, and your feature will not appear until you
...
### Variant
Variants represents the possible variations of a flag. Because flagr is
currently only used to toggle features, the only supported variant is `on`, and
this value is checked without taking casing into account.

### Segment
Segments represent the portions of our audience that your feature will be
enabled for. Each segment can have multiple conditions which **all** of which
...
| Property                        | Value Types  | Notes                       |
| :-----------------------------  | :----------- | :-------------------------- |
| `organization_id`               | `int`        |                             |
| `organization_slug`             | `string`     |                             |
| `organization_is-early-adopter` | `bool`       | Users opt in from settings  |
| `features`                      | `[string]`   | All active features for org |
| `subscription_is-free`          | `bool`       |                             |
| `subscription_is-trial-plan`    | `bool`       |                             |
...
| `user_is-staff`                 | `bool`       |                             |
| `internal_teams`                | `[string]`   | Only passed for Sentry      |

More properties can be added by updating `FlagrFeatureHandler.get_context`
but be considerate of the performance of additional context. As well properties
cannot contain periods `.` as this is a special character used by flagr.

### Operators
Generally operators are straight forward, for example `==` means equal. There is
the exception however of `CONTAINS` and `IN` which only operate on arrays, This
means that if you want to check that for a substring you want to use the `=~`
operator.
...
Values that are bools or ints do not need to be wrapped in quotes, for example
`true` or `1`
...
### Releasing a feature to a specific team in sentry

You can enable a feature for only a specific team within sentry by setting the
condition property to `internal_team`, the operator to `CONTAINS` and the value
to the slug of the team wrapped in double quotes `"`.
...
segment with the property set to `subscription_plan`, the operator to `IN` and
the value to an array of plans
...
increase it, enter the larger value and click *Save Segment Setting*.
...
- If the feature was enabled through flagr, remove it from `FlagrFeatureHandler.allowed_features`, and delete the feature from the Flagr UI as well this is done by navigating to the flag configuration page then clicking **Delete Flag** at the bottom of the page.
