...
Feature flags are controlled in Sentry at the commit and configuration level. At this time there is no built-in way to modify them in production in real-time.
...
If your feature flag doesn't have an 'off' mode, remove it and all related checks.
Otherwise, disable the feature by default in getsentry in `getsentry/conf/settings/defaults.py`,
add the necessary conditional in `getsentry/features.py`, and then enable it by default in
[`conf/server.py`](https://github.com/getsentry/sentry/blob/master/src/sentry/conf/server.py) to enable
the feature for users of self-hosted Sentry.
