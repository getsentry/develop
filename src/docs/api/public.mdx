...
- For our response serializers, you must type the `serialize` function's return with a `TypedDict` and pass the serializer as a parameter in `extend_schema`. See [here](https://github.com/getsentry/sentry/blob/master/src/sentry/api/serializers/models/organization_member.py) for an example of this.
...
- `make build-api-docs` will simply attempt to build the OpenAPI JSON. The build will fail if there are any warnings.
