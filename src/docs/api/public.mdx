...
sidebar tags. Note that the statment `response_only=True` is required for all examples and we
currently only support showing one response example per endpoint.
