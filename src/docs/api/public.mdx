...
Declare an owner for the endpoint. This would be the team at Sentry responsible
for maintaining and owning about the endpoint. You can see
the full list of owners [here](https://github.com/getsentry/sentry/blob/master/src/sentry/api/api_owners.py).
```python
...
Declare the appropiate endpoint methods public by setting their publish_status to `PUBLIC`.
