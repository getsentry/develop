...
Once you have Xcode installed, install [Homebrew](http://brew.sh), and then run `brew bundle` to install the various system packages as listed in sentry's `Brewfile`. This may take a few minutes, and brew isn't great about showing a progress bar, so it may look like the process has hung. Have faith and let it run.
...
Note that if you didn't already have node and yarn installed on your computer prior to this step, you'll need to run `volta install node@x.x.x` and `volta install yarn@x.x.x` when outside the `sentry` directory in order to use `node` and `yarn` elsewhere.
...
Now that you have sentry all set up, it's time to set up Getsentry. For information on the distinction between the two, refer to <Link to="/sentry-vs-getsentry/">Sentry vs Getsentry</Link>. After setting it up, you'll also want to read about the development workflow <Link to="/workflow/">here</Link>.
