...
One thing that requires manual attention is `docker`, which should have just been installed. You
can run this step for it to be set up for you or you can follow the manual steps below:
...
Note: If you use Fish, add `source (pyenv init - | psub)` to `~/.config/fish/config.fish`.
...
Two examples of variables that you can place there are `SENTRY_SILENCE_DIRENV_NOTICE` to make `direnv` output less noisy
and `SENTRY_DEVENV_NO_REPORT=1` to prevent the development environment to report errors to Sentry.
