...
You'll need to first install Xcode CLI tools. (If you try running `git` or `make`, you'll get an xcrun error). Run 
`xcode-select --install` and follow the instructions.

Once you have Xcode installed, install [Homebrew](http://brew.sh), and then run `brew bundle --verbose` to install
...
You can verify that Docker is running by running `docker ps` in your terminal. If it doesn't error with something
like `Error response from daemon: dial unix docker.raw.sock: connect: connection refused`, you're good to continue.
...
To install the required versions of Python you'll need to run `make setup-pyenv`. This will take a while, since your
