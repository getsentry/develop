...
Some useful variables to set in `.env`:

- `SENTRY_SILENCE_DIRENV_NOTICE`: Make `direnv` output less noisy.
- `SENTRY_DEVENV_NO_REPORT`: Do not report development environment errors to Sentry.io.
- `SENTRY_DIRENV_DEBUG`: Print debug messages to stdout.

A good starter `.env`:
```shell {filename: .env}
SENTRY_SILENCE_DIRENV_NOTICE=1
