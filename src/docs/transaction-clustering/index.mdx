...
- In post processing, we add incoming transaction names of type `url` to a project-specific set in redis.
...
- Every hour, a Celery task is spawned for every redis set, which
   - builds the tree and derives rules from merged nodes.
   - writes the rules into project options.
- The derived rules are written into the project config submitted to Relay.
- For every incoming transaction of type URL, Relay applies matching rules to scrub identifiers.
