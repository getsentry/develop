...
```URLs
/account/settings/nf215j634705291cvcm725757m53451365023487cv562n905n2634590xn26340/
/issues/1234567890
/saml/acs/my-organization/
/settings/e1fb593b4c3a4d388e2f6ea8c92c3abc/
```

### << unparameterized >>

Secondly, ingesting a high number of one-off transaction names does not scale to the required volume,
...
An intermediate solution to the scal problem was to drop the transaction tag from Performance metrics
...
free-form user names cannot be dinstinguished from low-cardinality parts of a URL.
...
1. In order to prune unused rules, we keep a copy of discovered rules in redis, and in post processing
...
The current threshold which decides whether or not a path segment is considered an identifier is set at 200.
We increased it from 100 because we noticed there are websites which have > 100 static pages.

For the sample of observed transactions, a larger set increases the probability of discovering a rule.
The current size is 2000 (10x the merge threshold).
