...
Our A/B testing framework is built on [Facebook's planout library](http://facebook.github.io/planout/).
...
- You can use other random assignment operators. Learn more: [https://facebook.github.io/planout/docs/random-operators.html](https://facebook.github.io/planout/docs/random-operators.html)
...
class Config(AppConfig):
		...
		.
		.
		from getsentry.planout.show_plugins_experiment import ShowPluginsExperiment
		.
		.
    expt_manager.add(ShowPluginsExperiment, "variant")
```

### Changes to **`getsentry/sentry`**

In `src/sentry/static/sentry/app/types/index.tsx` :
...
### Alternatives to Logging the Experiment

If the volume of analytics events is too high, we can track experiments indirectly via other methods. For example, if your test impacts the