...
docker-compose run --rm -T postgres bash -c "/usr/local/bin/docker-entrypoint.sh postgres & apt update && apt install -y --no-install-recommends postgresql-9.6-repack && su postgres -c 'pg_repack -E info -t nodestore_node'"
```
