...
This page provides a guide to help users secure their Sentry deployments and make the most out of Sentry. For further guide relating system security and performance, you might want to consult with a professional sysadmin.
...
We recommend TLS termination to be done on your own dedicated load balancer. Although you can set it on the `nginx.conf` file, it is not recommended as newer self-hosted releases might alter some configurations on the file. Some examples are available on [Using Dedicated Load Balancer](#using-dedicated-load-balancer) section.
