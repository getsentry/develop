...
All in-app page loads will generate analytics events by default (both Reload and Amplitude). All analytics related to page load should be handled within the route based analytics system instead of manually instruming front-end events. The default names are based on the parameterized path and are prefixed with `Page View`. Here is an example event name:
...
You will need to import both `withRouteAnalytics` and `WithRouteAnalyticsProps`. `WithRouteAnalyticsProps` needs to be added to the Props of the class. The functions can then accessed through the Props. For example, `this.props.setEventNames('issue_details.viewed', 'Issue Details: Viewed")`. `withRouteAnalytics` is called while exporting the class.
