...
All in-app page loads will generate analytics events by default (both Reload and Amplitude). All analytics related to page load should be handled within the route based analytics system instead of manually instrume front-end events. The default names are based on the parameterized path and are prefixed with `Page View`. Here is an example event name:
...
Route based analytics can be customized to have different event names add parameters. For class based components, `withRouteAnalytics` can be used. For functional components, the following hooks can be used:
...
The main purpose of customizing the event names is to create more readable event names that don't won't change if the underlying route changes.
