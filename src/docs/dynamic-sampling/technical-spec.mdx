...
All of these calculations are made periodically with a sliding window in order to account for changes in the incoming volume.

</Alert>

### Prioritize Low Volume Projects

This bias is used to prioritize low volume projects that can be drowned out by high volume projects. The goal is to rebalance sample rates of the individual projects so that low volume projects are more likely to contain transactions.
