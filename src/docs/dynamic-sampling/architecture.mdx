...
Relay is the first component involved in the Dynamic Sampling pipeline. It is responsible for receiving events from SDKs, sampling them, and forwarding them to the Sentry backend. _In reality, Relay does much more than that, but for the purpose of this document, we will focus on the sampling part._
...
A **rule** is the core component of the sampling configuration and is defined in the following JSON format (defined [here](https://getsentry.github.io/relay/relay_sampling/struct.SamplingRule.html)).
...
An illustration on how Redis is used for tracking boosted releases:
