...
- `condition`: used to express specific conditions on the incoming payload (the actual definition is available [here](https://getsentry.github.io/relay/relay_sampling/enum.RuleCondition.html)).
- `timeRange`: used to define the time range in which the rule is active.
- `decayingFn`: used to define the decaying function that will be applied to interpolate between the `samplingValue` and `decayedValue` (the condition `samplingValue` >= `decayedValue` must hold). There currently two decaying functions supported, namely `constant` and `linear`.
