...
At the core of Dynamic Sampling there is the concept of **fidelity**, which translates to an overall target sample rate that should be applied across all transactions of an organization. Within this target sample rate, Dynamic Sampling can create a **bias toward more meaningful data**. This is achieved by constantly updating and communicating special rules to Relay, via a project configuration, which then applies targeted sampling to every event.
...
The latest release bias uses a decaying rule to interpolate between a starting sample rate and an ending sample rate over a time window that is statically defined for each platform (the list of time to adoptions is define [here](https://github.com/getsentry/sentry/blob/master/src/sentry/dynamic_sampling/rules/helpers/time_to_adoptions.py#L26-L26). For example, Android has a bigger time window than Javascript because Android apps take more time to get adopted by users.
...
This bias is the simplest one that can be defined. It applies to any incoming event and is defined on a per-project basis.

_The sample rate of the uniform bias is computed using an algorithm revolving around the base sample rate, known as the target fidelity rate. This rate is defined at the organizational level and is return by the `quotas.get_blended_sample_rate` function._
