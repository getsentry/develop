...
If logging in a particular module is not showing up when you expect it to, you should check the log level for that module in `src/sentry/conf/server.py` with the `LOGGING` variable.

<Alert title="Corpoate Users Only" level="warning">
If you are a Sentry employee who is looking for logs in Kibana and they aren't showing up, one reason is because a field in the log message does not match Elasticsearch's schema. See the index patterns <a href="https://kibana.getsentry.net/app/kibana#/management/kibana/indices/0ba261d0-97ca-11e9-90db-dfacda4da93b?_g=()&_a=(tab:indexedFields)">here</a>.
</Alert>
