...
This means that if a trace is propogated from a running transaction _BEFORE_ the user/transaction attributes are set, you'll get a portion of transactions in a trace that have different Dynamic Sampling Context than other portions, leading to dynamic sampling not working as expected for users.

Let's say we want to dynamically sample my browser application based on the `user_id`.
...
It will _ONLY_ be able to dynamically sample the pageload transaction, as well as the transactions from service A, service B, and service C with a rule based on `user_id`.
