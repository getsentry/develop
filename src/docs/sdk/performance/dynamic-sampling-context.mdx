...
UX wise for the Dynamic Sampling product, we depend on tansaction names (i.e. the `transaction` parameter of the DSC) to have a "good quality".
**Good quality transaction names** are descriptive, have proper grouping on Sentry, have low cardinality, and do not contain PII.
...
- `transaction` (string, **only inclue iff name has [good quality](#note-on-low-quality-transaction-names)**) - The transaction name set on the scope.
...
- `sentry-environment`
- `sentry-user_segment`
- `sentry-transaction` (**only inclue iff name has [good quality](#note-on-low-quality-transaction-names)**)
