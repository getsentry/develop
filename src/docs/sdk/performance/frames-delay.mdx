...
* __Slow Frames__: Using 60 fps, slow frames are frames that take more than 16 ms (Android) or 16.67 ms
(iOS) to render.
...
Scenario: Missing delayed frame information
    """
...
Scenario: Delayed frame starts before time interval
    """
    [----  delayed frame ---- ]
            [- time interval -]
    """
    ...
    When calculating the frames delay for the time interval 100 ms to 300 ms
    Then the frames delay is the full queried time interval of 200ms


Scenario: Delayed frame starts shortly before time interval
    """
    ...
    Given a frame rate of 60 fps
    And a recorded delayed frame with a duration of 300 ms
    And the delayed frame started at 0 ms shortly before the time interval
    When calculating the frames delay for the time interval 10 ms to 300 ms
...
Scenario: Delayed frame starts and ends before time interval
    """
    [| e |  delayed frame ------ ]      e = the expected frame duration
        [--- time interval --- ]
    """
    ...
    Then the frames delay is 300 ms - 6.67 ms = 293.33 ms, which is only the delayed part of
        the delayed frame having an intersection with the time interval
    And the expected frame duration part of the delayed frame is not added as a frame delay


Scenario: One delayed frame starts shortly before end of time interval
    """
    ...
    When calculating the frames delay for the time interval 0 ms to 300 ms
