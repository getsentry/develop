...
1. Add an early return to `Sentry.startTransaction` and `Sentry.trace` (a helper method that ) if the `instrumentor` option is set to `otel`. This may require SDK changes, as `trace` is not part of the unified API.
...
class Hub implements HubInterface {
  ...
  startTransaction(
    ...
    // ...
    if (this.instrumentor !== context.instrumentor) {
      return;
    }
    // ...
  }
