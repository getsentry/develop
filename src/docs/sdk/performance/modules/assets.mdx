...
| `resource.render_blocking_status`      | string | wether the resource blocks UI rendering, values are 'non-blocking' or 'blocking' | recommended, if not provided the asset can't be narrowed down by their render blocking status |
