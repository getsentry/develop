...
Scenario: App started
    ...
    And the SDK will update the `time_to_initial_display` and `time_to_full_display` measurements accordingly
...
Scenario: API not called
    ...
    And the SDK will update the `time_to_full_display` measurement accordingly
