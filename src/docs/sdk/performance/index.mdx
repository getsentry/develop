...
Transactions should have a max number of spans. If new spans are added once the max is reached, the SDK should drop them and ideally use the internal logging to help debugging, `maxSpans` defaults to 1000.
...
- `Span` Interface
  ...
  - The `startChild` method should respect the `maxSpans` configuration, when the max. is reached the SDK should drop them and ideally use the internal logging to help debugging.
