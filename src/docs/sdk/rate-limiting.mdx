...
- `namespaces`: Semicolon-separated list of metric namespace identifiers. This will only be present if the rate limit applies to the `metric_bucket` data category.
...
HTTP/1.1 429 Too Many Requests
Retry-After: 2700
  2700:metric_bucket:organization:quota_exceeded:custom
```
...
While these [data categories](https://github.com/getsentry/relay/blob/master/relay-common/src/constants.rs#L97-L115) might look similar to the [envelope item types](/sdk/envelopes/#data-model), they are not identical, and have slight differences.
  ...
  - `metric_bucket`: Sentry Metrics sent via the `statsd` or `metrics` items. The _namespace_ component of the quota limit defines which namespace is affected.
