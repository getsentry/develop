...
- The SDK must support receiving HTTP status `429` without `X-Sentry-Rate-Limits` and `Retry-After` headers. In this case, use 60 seconds as a default value for `Retry-After`.
