...
- `reason_code`: A unique identifier for the quota hinting at the rate limiting reason.
...
- SDKs may ignore dimensions they do not support or are irrelevant, for example `scope` and `reason_code`. The `scope` can be ignored as long as the SDK only sends events to a single DSN.
...
- Limits where **all** categories are unknown must be ignored since those limits apply to data the SDK cannot send. Do not apply unknown categories to a default category. Note that this is distinct from limits with no category, which implicitly apply to all categories (think of this as a magic category).
...
- Unknown dimensions must be ignored, that is, all additional colons and text to the right of the last parsed/interpreted dimension.
