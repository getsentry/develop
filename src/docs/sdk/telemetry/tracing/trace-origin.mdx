...
the <Link to="/sdk/protocol/event-payloads/contexts/#trace-context">trace context</Link> or <Link to="/sdk/protocol/event-payloads/span/">spans</Link>.
...
: _Optional_. Is the category of the trace or span, and it must be a category of <Link to="/sdk/telemetry/tracing/span-operations/#currently-used-categories">span operations</Link>. For example, `http`, `db`, `ui`, `navigation`, `file`, `browser`, `rpc`, etc.
...
Origins of type `manual` can also have categories and integration names. For example, for the <Link to="/sdk/telemetry/tracing/time-to-initial-full-display/">time to display feature</Link>, the user manually calls an API  of the SDK,
...
- <Link to="/sdk/protocol/event-payloads/span/">Span Interface</Link>
- <Link to="/sdk/protocol/event-payloads/contexts/#trace-context">Trace Context</Link>
