...
* **Namespaces and Units:** regex `s/[^\w]+//g` with no replacement character.
* **Metric Keys/Names:** regex `s/[^\w\-.]+/_/g` with replacement character `_`.
* **Tag Keys:** regex `s/[^\w\-.\/]+//g` with no replacement character.
* **Tag Values:** See below

### Tage Values Replacement Map

Reserved characters for **Tag Values** are `\n`, `\r`, `\t`, `\`, `|`, `,`. Based on the above escaping rules, the replacement map is as follows.
