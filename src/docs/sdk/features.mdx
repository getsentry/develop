...
Add breadcrumb for each outgoing HTTP request after the request finishes:

- type: `http`
- category: `http`
- data: `url`, `method` (uppercase HTTP method), optionally `requestBodySize` and `responseBodySize` (in bytes)

If the Performance Monitoring is supported by the SDK and enabled in the client application, when the transaction is active, a new `Span` must be created around the HTTP request.
