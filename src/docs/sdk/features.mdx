...
If Performance Monitoring is both supported by the SDK and enabled in the client application when the transaction is active a new `Span` must be created around the HTTP request; The `Span#operation` should be `http.client` and the `Span#description` should follow the convention `$METHOD $url` eg `GET https://sentry.io`.
