...
The SDKs on these platforms send envelopes on a background thread to not block the UI thread. A crash occurring shortly
after the SDK init could lead to never reporting such crashes, keeping the users unaware of a critical bug. 
