...
- data (all fields are optional but recommended):
  ...
  - `http.query` - The URL query
  - `http.fragment` - The URL fragment
...
The GraphQL Client integrations match the [HTTP Client Integrations](/sdk/features/#http-client-integrations) with a few differences:

The `failedRequestStatusCodes` parameter does not exist because GraphQL errors are not HTTP errors, sometimes the request is errored and the response is successful.

Instead, the error has to be captured if the GraphQL response contains an `errors` array, you can do that by Regexing the response body, e.g.:
...
Additional fields for the Request interface:
...
Additional fields for the Response interface:
...
Additional fields for the Event interface:
