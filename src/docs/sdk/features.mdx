...
The `request.data` should be set with the raw GraphQL request payload, if possible.

The `contexts.response.data` should be set with the raw GraphQL response payload, only if there were `errors`.
