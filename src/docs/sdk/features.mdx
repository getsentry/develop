...
* If `captureFailedRequests` is disabled.
* If the HTTP Request url isn't a match for any of the `failedRequestsTargets`.
* If the HTTP Response status code isn't within the defined ranges in `failedRequestStatusCode`.
...
If the HTTP Client integration does not throw an error for unsuccessful requests, you can create a synthetic exception following this Spec:
