...
The transaction's name should be set with the GraphQL operation name, if possible, otherwise fallback to something unique that makes sense.

The transaction's description should be set with the GraphQL operation name, operation type and status code, if possible.

The `request.api_target` should be set to `graphql`.

The `request.data` should be set to the raw GraphQL request payload, if possible.

The `contexts.response.data` should be set to the raw GraphQL response payload, only if there were `errors`.
...
The operation type should follow the [Span Data Conventions](/sdk/performance/span-data-conventions/).

Instrumenting APM for GraphQL will depend on the instrumented GraphQL library, if there are available hooks for it, the SDK should use them, otherwise, the SDK could try to monkeypatch the library or instrument the transport layer using heuristics, for example, if the URL ends with `graphql`, etc.
