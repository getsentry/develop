...
* `failedRequestStatusCode` defaults to `500` to `599`, this param accepts a `List` of `HttpStatusCodeRange` which is a range of HTTP status code -> `min` to `max`.
* `failedRequestsTargets` defaults to `.*`, this param accepts a `List` of `String` that may be Regular expressions as well, similar to `tracePropagationTargets`.

Rules for automatically capturing error events.
...
See the <Link to="/sdk/event-payloads/request/">Request interface</Link> for details.
...
See the <Link to="/sdk/event-payloads/exception/">Exception Interface</Link> for details.

If the HTTP Client integration does not throw an error for unsuccessful requests, you can create a synthetic exception following this recommendation:
...
When capturing the error event, pass the original `Request` and `Response` objects from the HTTP Client as `hints`, so users may filter out this event with more context.

Those events can be searchable and alertable by the `http.url` and `http.status_code` properties, learn more about it in the [Searchable Properties](https://docs.sentry.io/product/sentry-basics/search/searchable-properties/) docs. 
