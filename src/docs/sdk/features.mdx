...
* `failedRequestStatusCodes` defaults to `500` to `599`, this configuration option accepts a `List` of `HttpStatusCodeRange` which is a range of HTTP status code -> `min` to `max` or a single `status_code`.
...
* If the HTTP Response status code isn't within the defined ranges in `failedRequestStatusCodes`.
