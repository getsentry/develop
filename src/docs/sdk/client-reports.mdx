...
```json
{
  ...
  "discarded_events": [
    ["queue_overflow", "error", 23],
    ["queue_overflow", "transaction", 1321]
  ]
}
```
...
: _List of outcome tuples_ [`discard_reason`, `data_category`, `quantity`]

- `discard_reason`: A string reason that defines why events were lost.
- `data_category`: The data category for which the discard reason applies.
