...
- adding `sentry-trace` and `baggage` HTTP headers (when doing outgoing HTTP requests) or 
...
- reading `sentry-trace` and `baggage` meta data when retrieving an item from a queue
- or reading the environment variables `SENTRY_TRACE` and `SENTRY_BAGGAGE` on start up.

This trace information should be stored in the "propagation context" of the current scope. This makes sure that all telemetry that is emmited to Sentry will include the trace information.
