...
If you do custom instrumentation with the SDK or are just curious what we are up to, continue on reading. Otherwise you can stop reading this page and go on with your day.
</markdown></Alert>
...
We have different concepts like a `Hub` and `Scope`s in the Unified API and those concepts sometimes leak to the end user of our SDKs. Those concepts are complicated and the users should almost never need to think about what a `Hub` or a `Scope` is and if they should clone them or not. 
...
There will be no `Hub` but we will have three different kinds of `Scope`s (that the end user has not to think about it. Only SDK and SDK integration developers need to understand them. So probably you!)
