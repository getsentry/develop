...
The profile data consists of a list of samples referring to stacks which in
terms refers to frames as well as some metadata about threads. This is to keep the amount of data we transfer to the
server as limited as possible.
...
- `elapsed_since_start_ns`: *String, required.* Contains the relative timestamp in nanoseconds since the value contained
in the `timestamp` field when the sample was captured.
...
We expect the stack to be from outermost frame to innermost (which means the
root frame is at the bottom of the stack).
...
Technically, all attributes are optional so we won't reject the payload if some
are missing.

Now, for native platforms (`cocoa` or `rust` for example),
`instruction_addr` is required in order to be able to symbolicate and get more
