...
The `/store` endpoint is deprecated. All data should be sent to the `/envelope` endpoint. See [Envelopes](/sdk/protocol/envelopes/).
...
For a list of all supported attributes and interfaces in event payloads, see <Link to="/sdk/protocol/event-payloads/">Event Payloads</Link>.
