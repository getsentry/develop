...
-   Event sampling
-   Honor Sentry’s <Link to="/sdk/endpoints/rate-limiting/">Rate Limiting</Link> HTTP headers
...
- <Link to="/sdk/protocol/envelopes/">/envelope/</Link> for any submission using Envelopes.
...
: The unix timestamp representing the time at which this event was generated.<br/>  
  *This key is effectively deprecated, and it is ignored on the receiving side.
  Use the [`sent_at` envelope header](/sdk/protocol/envelopes/#envelope-headers) instead.*
