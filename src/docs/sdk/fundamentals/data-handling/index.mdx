...
- [`before-send` and `event-processors`](/sdk/fundamentals/unified-api/#static-api) can be used to register a callback with custom logic to remove sensitive data.
...
- <Link to="/sdk/protocol/envelopes/#size-limits">Envelope Endpoint Size Limits</Link>
- <Link to="/sdk/endpoints/store/#size-limits">Store Endpoint Size Limits</Link>
