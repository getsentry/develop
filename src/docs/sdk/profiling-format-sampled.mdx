...
It's encouraged to de-duplicate stacks by only storing them once and referring to
...
envelope as the associated transaction with the `ItemType` `"profile"` and send
it to Relay.
