...
- Introduce `Sentry.startTransaction`
    ...
    - Users need to interaction with the instance and keep track of it themselves
    - Internally we can use the `Scope` to hold on the a reference of it if needed
- Introduce the new type `Span` → [https://develop.sentry.dev/sdk/event-payloads/span/](https://develop.sentry.dev/sdk/event-payloads/span/)
- Introduce new type `Transaction` which inherits from `Span`
- `Span` Interface
    - When a `Span` is created, set the `startSimestamp` to the current time
...
- `Hub` → Introduce a method called `startSpan`
    ...
    - This method deals with sampling, and therefore it should take the `tracesSampleRate` option into account:
        - If it's a child `Span`, we inherit the decision from the parent
        - If it's a root span, we need to roll the dice here
        - Depending on the outcome, the sample decision should be stored in the `Span`'s `sampled` property and again forwarded to its children
...
- `Scope` Introduce `setSpan`
    - This can be used internally to pass a `Span` / `Transaction` around so integration can attach children to it
    - The `transaction` property on the `Scope` (legacy) should overwrite the name of the `Transaction` if we send one. With that we give the user the option to change the name downstream if they don't have access to the instance of the `Transaction` directly.
- `Span.finish()`
...
- `Transaction.finish()`
    ...
    - `Transaction` needs to be wrapped in an `Envelope` and send through the [Envelope Endpoint](https://develop.sentry.dev/sdk/envelopes/)
...
- A `Transaction` should **not** go through `beforeSend` (but `EventProcessors` are fine for now, since they are considered internal)
    ...
    - Also, a few other properties see → Link to develop docs
    - **Motivation**: This shouldn't have been there in the first place. If we ever want to move to individual Span ingestion, we can't users rely on using this for Transaction. Since Tracing is still considered beta, we are ok "breaking" this for now if users relied on that.