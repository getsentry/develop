...
Events are packed into [envelopes](/sdk/protocol/envelopes/) and are sent to the `/api/{PROJECT_ID}/envelope/` API endpoint.
...
Sessions, client reports, replays, check-ins, and profiles are not events and have different size limits. See [Envelope Size Limits](/sdk/protocol/envelopes/#size-limits).
