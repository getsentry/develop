...
- In events reported from a Python/ASP.NET/Rails web backend, the OS context
  ...
  from the event's [Request Interface](/sdk/protocol/event-payloads/request/) and create a _secondary_ OS
...
The following example illustrates the contexts part of the <Link to="/sdk/protocol/event-payloads/">event payload</Link> and omits other attributes for simplicity.
...
: _Optional_. Short code identifying the type of operation the span is measuring. For more details, see [Sentry's conventions around span operations](/sdk/telemetry/tracing/span-operations/).
...
: _Optional_. The [Dynamic Sampling Context](/sdk/telemetry/tracing/dynamic-sampling-context/).

- Example: `{ "trace_id": "12312012123120121231201212312012", "sample_rate": "1.0", "public_key": "93D0D1125146288EAEE2A9B3AF4F96CCBE3CB316" },`

`origin`

: _Optional_. The origin of the trace indicates what created the trace. For more details, see <Link to="/sdk/telemetry/tracing/trace-origin">trace origin</Link>.
