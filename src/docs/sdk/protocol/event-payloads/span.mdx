...
A [Transaction](/sdk/protocol/event-payloads/transaction/) may contain zero or more Spans in an array attribute named `spans`. Spans in the list don't have to be ordered, they will be ordered by start / end time on the Server.
...
The semantic conventions for the `data` field are described in <Link to="/sdk/telemetry/tracing/span-data-conventions/">Sentry's Span Convention Documentation</Link>.
...
: _Optional_. The origin of the span indicates what created the span. For more details, see <Link to="/sdk/telemetry/tracing/trace-origin">trace origin</Link>.

## Examples

The following example illustrates the Span as part of the [Transaction](/sdk/protocol/event-payloads/transaction/) and omits other attributes for simplicity.
