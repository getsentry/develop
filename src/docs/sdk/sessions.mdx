...
SDKs may track sessions in one of two modes:
- Individual Sessions
- Session Aggregates

Individual sessions are the general case, and are a good fit for (relatively short-lived) applications that are typically involving only a single user. Examples:

- a command-line utility like `craft`; every execution of a `craft` subcommand would report a single session to Sentry
- a user interacting with a mobile app
- a user loading a web site with their favorite browser

Session aggregates are a special case for batching up individual sessions for efficiency of reporting them. This mode is used when sending individual sessions would be undesirable or unpractical. That is typically the choice for programs that run for an arbitrarily long time and handle larger throughputs for different users, such as web servers, background job workers, etc. Note that for those types of application, a better definition of session matches the execution of a single HTTP request or task, instead of a single execution of the whole application process.
