...
> The distinct ID. Should be a device or user ID. The system automatically hashes
> this ID before storing it.

`seq` (optional)

> A logical clock. Defaults to the current UNIX timestamp in milliseconds
...
> for now. If not sent, the server will assume the current UTC timestamp. In the data
...
> received. This can be client controlled so, for instance, inactive time can be subtracted (seconds as float).
...
> states. When a session is moved away from `ok` it must not be updated anymore.
>
