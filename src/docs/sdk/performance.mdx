...
3. If `tracesSampler` returns `null` and `tracesSampleRate` is defined, `tracesSampleRate` will be used.
4. If `tracesSampler` is not defined, but there's a parent sampling decision, the parent sampling decision will be used.
5. If `tracesSampler` is not defined and there's no parent sampling decision, `tracesSampleRate` will be used.
