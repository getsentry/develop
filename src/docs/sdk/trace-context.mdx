...
This feature is under development and is not required for all SDKs supporting Performance Monitoring, yet.
...
Tracestate headers contain several vendor-specific opaque data. As per HTTP spec, these multiple header values can be given in two ways:

- Joined by comma: `tracestate: sentry=<data>, other=<data>`
- Repetition:
  ```
...
4. Strip trailing `=` since this is a reserved character
5. Prepend with `sentry=`, resulting in `sentry=<base64>`
6. Join into the header as described above

<Note>

By stripping trailing `=`, default base64 parsers may detect an incomplete payload. Choose a parsing mode that either allows for missing `=` or allows truncated payloads.
...
This is an extension of trace ID propagation covered by [Performance Guidelines](https://develop.sentry.dev/sdk/performance/#header-sentry-trace). Please consider the above spec as reference documentation. Anything that contradicts it is a mistake (or an out of date detail) in this document.

According to the Unified API tracing spec mentioned above, Sentry SDKs add an HTTP header `sentry-trace` to outgoing requests via integrations. Most importantly, this header contains the trace ID, which must match the trace id of the transaction event and also of the trace context below.
...
It is recommended that SDK's log modifications of attributes that would result in trace context changes (like `user.id`) when the trace context is frozen in order to simplify debugging of common dynamic sampling rules mistakes.
