...
- If the stack trace is part of an error, exception or crash, add it to the <Link to="/sdk/event-payloads/exception">Exception Interface</Link>.
- Otherwise, add it as a thread in the <Link to="/sdk/event-payloads/threads">Threads Interface</Link>.
...
: An optional instruction address for symbolication. This should be a string
  ...
  known image is defined in the <Link to="/sdk/event-payloads/debugmeta">Debug Meta Interface</Link>, then
