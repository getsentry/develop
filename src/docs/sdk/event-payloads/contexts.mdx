...
* In events reported from a Python/ASPNET/Rails backend app, the OS context
  ...
  from the event's [./request](Request Interface) and create a _secondary_ OS
...
* `"os"` key for the crashing device's OS.
* `"client_os"` key for an adjacent client device's OS (that is _not_ the
  crashing device) if it's important. Only the server sets this as part of
  User-Agent parsing, but SDKs can set this too.
* If in doubt, just send `"os"`. Any other keys are not searchable in the
  ...
  like `"server_os"` to clarify what you meant is not probably going to
