...
: _Optional_. Application install mode. For example `Unknown, Store, DeveloperBuild, Adhoc, Enterprise, Editor`.

`copy_texture_support`

: _Optional_. Support for various copy texture cases. For example `None, Basic, Copy3D, DifferentTypes, TextureToRT, RTToTexture`.

`rendering_threading_mode`

: _Optional_. Application's actual rendering threading mode. For example `Direct, SingleThreaded, MultiThreaded, LegacyJobified, NativeGraphicsJobs, NativeGraphicsJobsWithoutRenderThread`.

`target_frame_rate`

: _Optional_. Instructs the game to try to render at a specified frame rate. For example `-1`.
