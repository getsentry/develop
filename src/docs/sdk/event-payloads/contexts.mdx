...
Cloud Context describes certain properties of cloud provider the software is running.

The `type` and default key is `"cloud"`.

`cloud.provider`

: _Optional_. Name of the cloud provider. For example `aws`.
...
: _Optional_. The cloud account ID the resource is assigned to. For example `111111111111`.

`cloud.region`

: _Optional_. The geographical region the resource is running.

: For example `us-central1` or `us-east-1`.

`cloud.availability_zone`

: _Optional_. Cloud regions often have multiple, isolated locations known as zones to increase availability. Availability zone represents the zone where the resource is running. For example `us-east-1c`
