...
: _Optional_. Processor frequency in MHz. Note that the actual CPU frequency might vary depending on current load and power conditions, 
  especially on low-powered devices like phones and laptops. On some platforms it's not possible to query the CPU frequency. 
  Currently such platforms are iOS and WebGL. For example, `2500`.
...
: _Optional_. Unique device identifier. Depends on the running platform. For example, `iOS: UIDevice.identifierForVendor (UUID)`,
  `Android: md5 of ANDROID_ID`, `Windows Store Apps: AdvertisingManager::AdvertisingId (possible fallback to HardwareIdentification::GetPackageSpecificToken().Id)`,
