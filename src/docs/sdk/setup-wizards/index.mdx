...
If youcreate a new prompt, make sure you wrap it with the [`abortIfCancelled`](https://github.com/getsentry/sentry-wizard/blob/c5b7f72f306cde8e6516acc11a5c6b75acee60c6/src/utils/clack-utils.ts#L48) function.
...
- Use `clack.log.error` and optionally `chalk.red` for errors.
- Use `clack.log.info` along the way to print additional information.
  For example, if the wizard fails to modify a file or install a package.
- Use `clack.log.step` to print a message regarding completing or entering a new step that require justify a success message.
