...
It's possible to run Sentry locally without ever having to build the frontend. It requires to circumvent `sentry devserver`. Instead, you run:
...
In order to use the web interface, you still need a frontend. For that, you can configure the following in your `~/.sentry/sentry.conf.py`:
