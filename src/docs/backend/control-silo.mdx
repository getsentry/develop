...
<Alert level="info">
💡 Requests going to [sentry.io](http://sentry.io) do not meet data residency requirements. Customers will need to use region domains in order to not have requests go to the US.
</Alert>
...
Many of the 3rd-party services we integrate with only offer a single endpoint that they will submit webhooks to. Because all of our existing webhook traffic is being sent to `[sentry.io](http://sentry.io)` we need to receive webhook requests there. For webhooks that need to be handled in a region silo, we store the webhooks in postgres and then asynchronously deliver captured webhooks to the relevant regions.
