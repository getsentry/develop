...
To apply styles to HTML we use a css-in-js library called [Emotion](https://emotion.sh/). This allows us to define styles in the same file where the component is defined and used. Styles are scoped to avoid global selectors and having to manually specify classnames at strings. You can also leverage the theme object to reference common values and utilities.
...
Data selectors can be an easy way to set styles based on simple/pre-defined state. When defining these attributes don't forget that browser have built-in selectors like `:disabled`, `:focus`, `:first-child` and so on.
...
For more flexible values, you can use CSS variables instead of pre-defined data-attributes for some value types.

```tsx
import styled from '@emotion/styled';

// ✅ Define css variables for overridable properties:
const Label = styled('label')`
  color: var(--color, ${p => p.theme.textColor});
`;
...
const label = <Label>There is an error</Label>;
return isError
  ? <Error>{label}</Error>
  : <Info>{label}</Info>;
```
...
The `style` attribute is possible to use, but the values are not linted. Avoid setting too many values at a time for readability.
