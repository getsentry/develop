...
You can either grant a temporary exception in your browser, or generate some local SSL certificates and use your OS to mark them as "trusted". The instructions below will show you how to generate some local certificates.

We will be using [mkcert](https://github.com/FiloSottile/mkcert) to generate and install a locally-trusted, development certificate. The following will install `mkcert` and then create and install the local certificates.
