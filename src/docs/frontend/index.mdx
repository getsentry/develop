...
Run Storybook locally with `yarn storybook` or view the hosted version at `https://storybook.sentry.dev/`
...
Client.addMockResponse is the best way to mock API requests. it’s [our code](https://github.com/getsentry/sentry/blob/master/static/app/__mocks__/api.tsx#L94-L117) so if it’s confusing you, just put `console.log()` statements into its logic!
