...
The `useState`, `useMemo`, `useCallback`, `useContext`, and `useRef` are welcome
...
As we plan our path away from Reflux, the `useContext` hook provide a simpler
implementation option to share state and behavior. When you need to create new
sources of shared state, consider using context and `useContext` over reflux.
