...
Note that we add an `options` argument to our `useFetchEvents` hook so that consumers can pass their own options. Some options that can be helpful to use are:
...
function handleCreateProject() {
  ...
  setApiQueryData<ProjectsResponse>(queryClient, makeProjectsQueryKey(...), data => {
