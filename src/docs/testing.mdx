...
Endpoint integration tests based on `APITestCase` is where the bulk of our test suite is focused. These tests help us ensure that
...
* **Use `unittest.TestCase` if you do not need access to the database.** Those tests will set up and teardown very quickly.
* Use `sentry.testutils.TestCase` or `sentry.testutils.TransactionTestCase` for database access. Those are subtypes of the respectively named Django test classes.
* There are also a couple of domain-specific test classes, most importantly `sentry.testutils.APITestCase`.
