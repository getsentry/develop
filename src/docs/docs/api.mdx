...
In this document, we explain how you make an API public and the considerations you should take into account before you make it public.
...
Let's say we also return a `nickname` for a team. If what is returned is not documented as a response in our public documentation, that return is a private attribute within a public endpoint.
...
When you make an endpoint public, you're committing to always returning at least the set of defined attributes when that endpoint is called. As a result, the attribute(s) **cannot** **be removed** from an endpoint after they are public. Additionally, the type of attributed **cannot** **be changed** once the attribute is public. The reason the attribute and its type cannot be changed is because both in the wild and managed clients are relying on the public attribute to return what is expected from that endpoint.
...
1. Get the latest SHA from sentry-api-schema.
...
No problem. Ping the Ecosystem Team.
