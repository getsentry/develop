...
What constitues as inclusive language will always be an evolving topic and
because Sentry is a product with history it's likely that we will always carry
something in the code base that is hard to get rid of.
...
that might still exist in parts of the code base.
...
We're okay keeping the term "master" for existing repositories but it's
...
that have already clonsed the repositories so these should not be changed.
...
If you identified code or comments that use problematic language you are free
to change it.  However please be advised that some uses of problematic language
haven't been phased out because they are backwards incomaptible changes and
