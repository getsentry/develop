...
- Tasks arguments _should_ be primitive types and small.
  ...
  pass an ID to the task which can be used to load the data from the
  database rather than the data itself.

  Similarly to keep the message borkers and workers running
  efficiently serlialising large values into the message results in
