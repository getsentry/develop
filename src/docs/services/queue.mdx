...
@retry(on=(<RetryableError>), exclude=(<reraise>), ignore=(<IgnorableError>))
...
- Tasks _should_ retry on exception.
  ...
  - Retryable exceptions: transient errors (e.g. connection timeouts, 
  DB unavailable, query read timeouts) 
  - Ignorable exceptions: this is task specific but an example of an 
  ...
  - Non-ignorable programming errors that should be re-raised and fixed 
