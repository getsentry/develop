...
- Relay (freight deployment = `relay`)

- External Relays (customers run those, we practically have no deprecation policy for old versions)

  - At some point we want to serve those requests from Relay, such that only internal Relays can hit Sentry. This would limit complexity.

Likewise *either Sentry or (internal) Relay* can forward your configuration to:
...
This should be generally OK as long as it serializes/deserializes the same, for example you can swap out `String` with `smartstring::String`.

Decreasing integer sizes (`u64` to `u32`) is a breaking change unless you can somehow prove all possible values fit into the new integer type, else it cannot be done without [renaming the field](#breaking-changes-and-actual-versioning). Serde throws an error if the value-to-deserialize does not fit into the integer type.

Increasing integer sizes (`u32` to `u64`) should be fine in most cases. Just keep in mind that when you increase the integer size and deploy internal relays, you push potential overflow errors further to the edge (or customer) where you no longer see them.

In case your field is emitted for external Relays, **changing a field type is rarely possible** without [renaming the field](#breaking-changes-and-actual-versioning) entirely. Even if you think it's feasible, renaming the field in addition is often a worthwhile additional security dance.

In case it's not emitted to external relays (not part of `LimitedProjectConfig`), you may be able to at least extend the set of types Relay accepts. For that it's recommended to either:

- [Use an untagged enum](https://serde.rs/enum-representations.html#untagged) (this is like `Union[str, int]` in Python)
- Write a custom `Deserialize` impl for your type to be able to deserialize from older values

## Deployment & Monitoring

Please refer to [this internal Notion page](https://www.notion.so/sentry/Naming-and-versioning-patterns-in-project-config-6486b568d0f0453aa498db908486671b?d=028f20b0-196d-481c-bf69-7d5d03c32a33#7f59d3cf49864969b9709ba8467fc00d) for Sentry.io specific deployment information.

## Breaking changes, and actual versioning

> `myfield_v2_final_final.docx`
