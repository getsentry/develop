...
- Generally API URLs should not be required to change. Existing customers can use `[region.sentry.io](http://region.sentry.io)` style URLs to improve latency and ensure data residency requirements are met, but they don’t need to. Customers in new regions may be required to use `region.sentry.io` style URLs.
